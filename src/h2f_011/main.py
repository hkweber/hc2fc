...
class HC2FCApp:
    ...
    '''
        ...
        for col in data.columns:
            if "_offset" in col:
                filtered_data[col] = data[col]  # ✅ Ensure offset columns are preserved
    ...
    def store_filtered_data(self, dataset_type):
        ...
        # Apply filters and modifications to the dataset
        filtered_data = self.apply_filters(dataset_type)
        if filtered_data is None or filtered_data.empty:
            messagebox.showerror("Error", f"No data available after filtering {dataset_type}.")
            return

        # Debugging: Check if the modified columns exist
        print("Storing filtered dataset with columns:", filtered_data.columns)

        # Generate dataset name with filters and modifications
