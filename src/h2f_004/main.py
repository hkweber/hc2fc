...
class HC2FCApp:
    ...
    def _initialize_ui(self):
        ...
        self._create_key_values_widgets()
#        self._create_modify_data_widgets()
        self._create_modify_section()
        self._create_browser_section()
#        self._create_filtered_data_browser()
    ...
    def _create_project_section(self):
        """
        Create UI elements related to project management (save, load, view plots).
        """
        ...
        tk.Button(project_frame, text="Save Project", command=self.save_project_callback, font=UIStyling.BUTTON_FONT).pack(side="left", padx=UIStyling.PAD_X, pady=UIStyling.PAD_Y)
        tk.Button(project_frame, text="Load Project", command=self.load_project_callback, font=UIStyling.BUTTON_FONT).pack(side="left", padx=UIStyling.PAD_X, pady=UIStyling.PAD_Y)
        tk.Button(project_frame, text="Show Loaded Plots", command=self.show_loaded_plots, font=UIStyling.BUTTON_FONT).pack(side="left", padx=UIStyling.PAD_X, pady=UIStyling.PAD_Y)

    def _create_data_widgets(self):
        """ Create data selection widgets for anode and cathode. """
        data_section = tk.Frame(self.main_frame)
        data_section.pack(fill="x", padx=UIStyling.PAD_X, pady=UIStyling.PAD_Y)
    ...
    def _create_filter_widgets(self):
        """ Create filter widgets for anode and cathode datasets. """
        filter_section = tk.Frame(self.main_frame)
        filter_section.pack(fill="x", padx=UIStyling.PAD_X, pady=UIStyling.PAD_Y)
    ...
    def _create_key_values_widgets(self):
        """ Create widgets for extracting key values. """
        key_values_section = tk.Frame(self.main_frame)
        key_values_section.pack(fill="x", padx=UIStyling.PAD_X, pady=UIStyling.PAD_Y)
    ...
    def _create_modify_section(self):
        """
        Creates a labeled section for modifying datasets.
        """
        modify_frame = tk.LabelFrame(self.main_frame, text="Modify Data", font=UIStyling.LABEL_FONT)
        modify_frame.pack(fill="x", padx=UIStyling.FRAME_PADX, pady=UIStyling.FRAME_PADY)

        anode_frame = tk.Frame(modify_frame)
        anode_frame.pack(side="left", fill="both", expand=True, padx=UIStyling.PAD_X)

        cathode_frame = tk.Frame(modify_frame)
        cathode_frame.pack(side="left", fill="both", expand=True, padx=UIStyling.PAD_X)

        self.anode_modify_widget = ModifyDataWidget(anode_frame, "Modify Anode Data", "anode", self)
        self.cathode_modify_widget = ModifyDataWidget(cathode_frame, "Modify Cathode Data", "cathode", self)
    '''
        def _create_filtered_data_browser(self):
        """ Create the filtered data browser section. """
        browser_section = tk.Frame(self.main_frame)
        browser_section.pack(fill="x", padx=UIStyling.PAD_X, pady=UIStyling.PAD_Y)
