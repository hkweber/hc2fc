...
class FilterWidget:
    ...
    def __init__(self, parent, label_text, dataset_type, app_context):
        ...
        self.selected_cycle_column = tk.StringVar(value="Cyc-Count")
    ...
    def _create_filter_options(self):
        ...
        # **New: Cycle Column Selection Dropdown**
        self.cycle_column_dropdown = tk.OptionMenu(cycle_frame, self.selected_cycle_column, "Cyc-Count")
    ...
    '''
        ...
        # Enable cycle selection dropdown
        self.cycle_dropdown.config(state="normal")

        # If abs_cycle is available, allow selection
        menu = self.cycle_column_dropdown["menu"]
        menu.delete(0, "end")
        menu.add_command(label="Cyc-Count", command=lambda: self.selected_cycle_column.set("Cyc-Count"))
        if abs_cycle_available:
            menu.add_command(label="abs_cycle", command=lambda: self.selected_cycle_column.set("abs_cycle"))

        self.cycle_column_dropdown.config(state="normal")

    '''
    def update_cycle_options(self, cycles, abs_cycle_available=False):
        """
        Updates the cycle dropdown options based on the dataset.
        Adds 'Cyc-Count' and 'abs_cycle' if available.
        """
        menu = self.cycle_dropdown["menu"]
        menu.delete(0, "end")  # Clear existing options
        menu.add_command(label="All", command=lambda: self.cycle_selection.set("All"))  # Add "All" option
        ...
        # ✅ If 'abs_cycle' is available, allow the user to select it
        self.cycle_column_option.set("Cyc-Count")  # Default selection
        menu = self.cycle_column_dropdown["menu"]
        menu.delete(0, "end")  # Clear existing options
        menu.add_command(label="Cyc-Count", command=lambda: self.cycle_column_option.set("Cyc-Count"))

        if abs_cycle_available:
            menu.add_command(label="abs_cycle", command=lambda: self.cycle_column_option.set("abs_cycle"))
