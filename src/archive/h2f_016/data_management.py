...
class DataManager:
    ...
    def load_dataset(self, dataset_type, file_path):
        ...
        try:
            ...
            # ✅ Check if 'abs_cycle' column exists
            abs_cycle_available = "abs_cycle" in data.columns  

            # Update cycle dropdown options if 'Cyc-Count' column exists
            unique_cycles = sorted(data["Cyc-Count"].unique()) if "Cyc-Count" in data.columns else []

            # ✅ Pass abs_cycle_available to update_cycle_options()
            if dataset_type == "anode":
                self.app.anode_filter_widget.update_cycle_options(unique_cycles, abs_cycle_available)
            else:
                self.app.cathode_filter_widget.update_cycle_options(unique_cycles, abs_cycle_available)
    ...
    def apply_filters(self, dataset_type):
        ...
        # ✅ Get selected cycle column
        cycle_column = widget.selected_cycle_column.get()
