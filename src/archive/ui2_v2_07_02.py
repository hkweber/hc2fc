...
class HC2FCApp:
    ...
    def plot_filtered_data(self, dataset_type):
        ...
        try:
            ...
            if show_key_values:
                key_points = self.compute_key_points(filtered_data, key_values_widget, plot_type)
    ...
    def save_filtered_data(self, dataset_type):
        ...
        base_filename = os.path.splitext(os.path.basename(dataset_file))[0]  # Just the base filename without the path

        # Generate suffix based on applied filters
        widget = self.anode_filter_widget if dataset_type == "anode" else self.cathode_filter_widget
        suffixes = []
        if widget.remove_pause.get():
        ...
        # Join suffixes with "-" (if any) and append to the filename
        suffix_string = f"-{'-'.join(suffixes)}" if suffixes else ""
        suggested_filename = f"{base_filename}{suffix_string}_filtered.csv"
        ...
        save_path = filedialog.asksaveasfilename(
            ...
            initialdir=save_folder,  # Start in the filtered_data folder
            initialfile=suggested_filename,  # Suggested filename includes suffixes
    ...
    def compute_key_points(self, filtered_data, widget, plot_type):
        """
        ...
        # Determine which key values are relevant based on the plot type
        if plot_type == "U-t":  # Voltage vs Time
            if widget.extract_max_voltage.get():
                max_voltage = filtered_data["U[V]"].max()
                time_at_max_voltage = filtered_data.loc[filtered_data["U[V]"] == max_voltage, "Time[h]"].iloc[0]
                key_points.append(
                    {"x": time_at_max_voltage, "y": max_voltage, "color": "blue", "marker": "o", "label": f"Max Voltage: {max_voltage:.2f} V"}
                )

        elif plot_type == "I-t":  # Current vs Time
            if widget.extract_max_charge.get():  # Assuming this refers to max current
                max_current = filtered_data["I[A]"].max()
                time_at_max_current = filtered_data.loc[filtered_data["I[A]"] == max_current, "Time[h]"].iloc[0]
                key_points.append(
                    {"x": time_at_max_current, "y": max_current, "color": "orange", "marker": "x", "label": f"Max Current: {max_current:.2f} A"}
                )

        elif plot_type == "Q-U":  # Charge vs Voltage
            if widget.extract_max_charge.get() and "Ah-Cyc-Charge-0" in filtered_data.columns:
                ...
                key_points.append(
                    {"x": max_charge, "y": voltage_at_max_charge, "color": "green", "marker": "x", "label": f"Max Charge: {max_charge:.2f} Ah"}
                )
            if widget.extract_max_discharge.get() and "Ah-Cyc-Discharge-0" in filtered_data.columns:
                ...
                key_points.append(
                    {"x": max_discharge, "y": voltage_at_max_discharge, "color": "red", "marker": "^", "label": f"Max Discharge: {max_discharge:.2f} Ah"}
                )
