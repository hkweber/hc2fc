...
class MultiGraphPlotter:
    ...
    def plot_multi_graph(self):
        ...
        for dataset_name in anode_datasets:
            ...
            if dataset is not None:
                self._plot_dataset(dataset, dataset_name, "blue")
        ...
        for dataset_name in cathode_datasets:
            ...
            if dataset is not None:
                self._plot_dataset(dataset, dataset_name, "red")
    ...
    def _plot_dataset(self, dataset, label, color):
        """
        Plot a single dataset on the current matplotlib figure.
        """
        try:
            if "Time[h]" in dataset.columns and "U[V]" in dataset.columns:
                plt.plot(dataset["Time[h]"], dataset["U[V]"], label=label, color=color)
            else:
                messagebox.showerror("Error", f"Dataset {label} does not have required columns for plotting.")
...
class HC2FCApp:
    ...
    def _initialize_ui(self):
        ...
        tk.Label(self.root, text="Full cell data section", font=("Arial", 12, "underline")).pack(pady=5)
        tk.Button(self.root, text="Plot Multi-Graph", command=self._plot_multi_graph).pack(pady=10)
