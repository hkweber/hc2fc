...
class HC2FCApp:
    ...
    def save_project(self, project_path):
        """
        Saves the current state of the application, including datasets, applied filters, and plots.
        Stores all data in a single `.hc2fcproj` zip archive.
        """
        project_data = {
            "datasets": {},  # Store dataset names and metadata (not raw data)
            "filtered_datasets": {},  # Store filtered datasets
            "filters": {}  # Store applied filters
        }

        # Store all datasets
        for dataset_type, dataset_info in self.datasets.items():
            if dataset_info["data"] is not None:
                project_data["datasets"][dataset_type] = {
                    "file_path": dataset_info["file_path"].get()
                }
        ...
        project_data["filters"]["anode"] = self.anode_filter_widget.__dict__
        project_data["filters"]["cathode"] = self.cathode_filter_widget.__dict__
        ...
        with zipfile.ZipFile(project_path, "w") as project_zip:
            ...
            for dataset_type, dataset_info in self.datasets.items():
                if dataset_info["data"] is not None:
                    ...
                    project_zip.writestr(f"{dataset_type}.csv", csv_data)

            # Save filtered datasets as CSV
            for dataset_type, datasets in self.filtered_datasets.items():
                for dataset in datasets:
                    csv_data = dataset["data"].to_csv(index=False)
                    project_zip.writestr(f"{dataset['name']}.csv", csv_data)

            # **SAVE PLOTS**: Capture and store all open Matplotlib figures as PNG
            for i, fig in enumerate(plt.get_fignums()):
                fig_obj = plt.figure(fig)
                img_buffer = io.BytesIO()
                fig_obj.savefig(img_buffer, format="png")  # Save plot to buffer
                img_buffer.seek(0)
                project_zip.writestr(f"plot_{i}.png", img_buffer.read())  # Write to zip
    ...
    def save_project(self, project_path):
        ...
        try:
            ...
            # Save datasets (but only their filenames)
            for dataset_type, datasets in self.filtered_datasets.items():
                project_data["datasets"][dataset_type] = []
                for dataset in datasets:
                    project_data["datasets"][dataset_type].append(dataset["name"])  # Store just the name

            # Save active filters (only storing relevant options)
            for dataset_type in ["anode", "cathode"]:
                ...
                project_data["filters"][dataset_type] = {
                    ...
                    "stepchange": widget.stepchange.get(),  # New Stepchange filter
                }
            ...
            with zipfile.ZipFile(project_path, "w", zipfile.ZIP_DEFLATED) as project_zip:
                ...
                # Save actual dataset CSV files
                for dataset_type, datasets in self.filtered_datasets.items():
                    for dataset in datasets:
                        dataset_name = dataset["name"]
                        dataset_data = dataset["data"]
                        dataset_csv = dataset_data.to_csv(index=False)

                        project_zip.writestr(f"datasets/{dataset_name}.csv", dataset_csv)
    ...
    def load_project(self, project_path):
        """
        Loads a .hc2fcproj file, restoring datasets, filters, and other settings.
        """
        try:
            with zipfile.ZipFile(project_path, "r") as project_zip:
                ...
                for dataset_type, dataset_list in project_data["datasets"].items():
                    for dataset_name in dataset_list:
                        ...
                        if dataset_csv_path in project_zip.namelist():
                            ...
                            print(f"Warning: Missing dataset {dataset_name} in project file.")
                ...
                for dataset_type in ["anode", "cathode"]:
                    ...
                    if dataset_type in project_data["filters"]:
                        ...
                        widget.stepchange.set(filter_data.get("stepchange", False))  # Restore stepchange filter
