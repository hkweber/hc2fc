...
class HC2FCApp:
    ...
    def save_project(self, project_path):
        ...
        """
        try:
            # Ensure filenames in metadata end with ".txt"
            project_data = {
                "original_files": {
                    "anode": self._ensure_txt_extension(self.datasets["anode"]["file_path"].get()),
                    "cathode": self._ensure_txt_extension(self.datasets["cathode"]["file_path"].get())
                },
                "datasets": {dtype: [self._ensure_txt_extension(ds["name"]) for ds in self.filtered_datasets[dtype]]
            ...
            with zipfile.ZipFile(project_path, "w", zipfile.ZIP_DEFLATED) as project_zip:
                ...
                for dtype in ["anode", "cathode"]:
                    file_name = self._ensure_txt_extension(self.datasets[dtype]["file_path"].get())
                    ...
                    if dataset is not None and file_name != "No file selected":
                        ...
                        project_zip.writestr(f"datasets/{file_name}", dataset_txt)

                # Save filtered datasets
                for dtype, datasets in self.filtered_datasets.items():
                    for dataset in datasets:
                        file_name = self._ensure_txt_extension(dataset["name"])
    ...
    def load_project(self, project_path):
        ...
        try:
            with zipfile.ZipFile(project_path, "r") as project_zip:
                ...
                # Clear existing datasets and filtered datasets
                self.datasets = {
                    "anode": {"data": None, "file_path": tk.StringVar(value="No file selected")},
                    "cathode": {"data": None, "file_path": tk.StringVar(value="No file selected")},
                }
                ...
                for dtype in ["anode", "cathode"]:
                    ...
                    dataset_txt_path = f"datasets/{file_name}"
                ...
                for dtype, dataset_names in project_data["datasets"].items():
                    for dataset_name in dataset_names:
                        dataset_txt_path = f"datasets/{dataset_name}"
                        if dataset_txt_path in project_zip.namelist():
                            with project_zip.open(dataset_txt_path) as dataset_file:
                                dataset_df = pd.read_csv(dataset_file, delimiter=",")
                                self.filtered_datasets[dtype].append({"name": dataset_name, "data": dataset_df})

                print("✅ Datasets and filtered datasets restored.")
    ...
    def select_file(self, dataset_type):
        ...
        if file_path:
            try:
                ...
                self.datasets[dataset_type]["file_path"].set(self._ensure_txt_extension(os.path.basename(file_path)))
    ...
    def store_filtered_data(self, dataset_type):
        """
        Stores the filtered dataset with a consistent filename and updates metadata.
        """
        ...
        dataset_file = self._ensure_txt_extension(self.datasets[dataset_type]["file_path"].get())
        dataset_name = f"{os.path.splitext(dataset_file)[0]}_filtered.txt"
    ...
    def _ensure_txt_extension(self, file_name):
        """
        Ensures that a filename ends with ".txt".
        """
