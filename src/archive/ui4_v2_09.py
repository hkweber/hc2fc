...
class HC2FCApp:
    ...
    def save_project(self, project_path):
        ...
        try:
            ...
            # Save datasets (only storing dataset names)
            ...
            # Save active filters (including the missing ones!)
            for dataset_type in ["anode", "cathode"]:
                ...
                project_data["filters"][dataset_type] = {
                    ...
                    "select_charge_half_cycle": widget.select_charge_half_cycle.get(),  # ✅ New
                    "select_discharge_half_cycle": widget.select_discharge_half_cycle.get(),  # ✅ New
                    "plot_type": widget.plot_option.get()  # ✅ New
                }
            ...
            with zipfile.ZipFile(project_path, "w", zipfile.ZIP_DEFLATED) as project_zip:
                try:
                    ...
                    print(f"Error serializing project metadata: {e}")
    ...
    def load_project(self, project_path):
        ...
        try:
            with zipfile.ZipFile(project_path, "r") as project_zip:
                    ...
                    for dataset_name in dataset_list:
                        ...
                        if dataset_csv_path in project_zip.namelist():
                            with project_zip.open(dataset_csv_path) as dataset_file:
                                ...
                                # ✅ Restore to main anode/cathode datasets
                                if dataset_type in ["anode", "cathode"]: 
                                    self.datasets[dataset_type]["data"] = dataset_df
                                    self.datasets[dataset_type]["file_path"].set(dataset_name)

                # Restore filters (including the missing ones!)
                for dataset_type in ["anode", "cathode"]:
                    widget = self.anode_filter_widget if dataset_type == "anode" else self.cathode_filter_widget
                    if dataset_type in project_data["filters"]:
                        ...
                        widget.select_charge_half_cycle.set(filter_data.get("select_charge_half_cycle", False))  # ✅ New
                        widget.select_discharge_half_cycle.set(filter_data.get("select_discharge_half_cycle", False))  # ✅ New
                        widget.plot_option.set(filter_data.get("plot_type", "U-t"))  # ✅ New

                # Restore datasets in the UI
                ...
                print("Datasets restored in browsers and main UI.")

                # Restore plots from ZIP
                self.loaded_plots = []
