...
class HC2FCApp:
    ...
    def compute_absolute_cycle(self, dataset):
        """
        Compute the absolute cycle column, ensuring cycle numbers remain continuous.
        """
        if "Cyc-Count" not in dataset.columns:
            messagebox.showerror("Error", "Dataset is missing 'Cyc-Count' column.")
            return dataset

        abs_cycle = []
        current_cycle = dataset["Cyc-Count"].iloc[0]  # Start with the first cycle
        prev_cycle = dataset["Cyc-Count"].iloc[0]

        for cyc in dataset["Cyc-Count"]:
            if cyc < prev_cycle:  # Cycle reset detected
                current_cycle += 1  # Increase absolute cycle number
            
            abs_cycle.append(current_cycle)
            prev_cycle = cyc  # Update previous cycle

        dataset["abs_cycle"] = abs_cycle  # Add the column to dataset
        return dataset
