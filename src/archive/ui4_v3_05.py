    ...
    def load_project(self, project_path):
        ...
        try:
            with zipfile.ZipFile(project_path, "r") as project_zip:
                ...
                self.datasets["anode"]["data"] = None
                self.datasets["cathode"]["data"] = None
                self.datasets["anode"]["file_path"].set("No file selected")  
                self.datasets["cathode"]["file_path"].set("No file selected")
                ...
                # ✅ Step 2: Restore original anode and cathode filenames
                if "original_files" in project_data:
                    self.datasets["anode"]["file_path"].set(project_data["original_files"].get("anode", "No file selected"))
                    self.datasets["cathode"]["file_path"].set(project_data["original_files"].get("cathode", "No file selected"))

                # ✅ Step 3: Restore filtered datasets from the project
                    ...
                    for dataset_name in dataset_list:
                        ...
                        if dataset_csv_path in project_zip.namelist():
                            with project_zip.open(dataset_csv_path) as dataset_file:
                                ...
                                self.filtered_datasets[dataset_type].append({"name": dataset_name, "data": dataset_df})
                ...
                # ✅ Step 4: **Force UI update for anode/cathode file labels**
                ...
                # ✅ Step 5: Restore filter settings from the project
                for dataset_type in ["anode", "cathode"]:
                    widget = self.anode_filter_widget if dataset_type == "anode" else self.cathode_filter_widget
                ...
                # ✅ Step 6: Restore cycle dropdowns
                for dataset_type in ["anode", "cathode"]:
                    dataset = self.datasets[dataset_type]["data"]
                    if dataset is not None and "Cyc-Count" in dataset.columns:
                        unique_cycles = sorted(dataset["Cyc-Count"].unique())
                        widget = self.anode_filter_widget if dataset_type == "anode" else self.cathode_filter_widget
                        widget.update_cycle_options(unique_cycles)  # Refresh dropdown options

                print("✅ Cycle dropdowns updated successfully.")

                # ✅ Step 7: Restore plots
                ...
                print("🎉 Project loaded successfully and previous data cleared!")
