...
class ModifyDataWidget:
    ...
    def __init__(self, parent, label_text, dataset_type, app_context):
        ...
        self.compute_abs_cycle = tk.BooleanVar()  # NEW Variable for Absolute Cycle
        ...
        tk.Checkbutton(self.frame, text="Compute Absolute Cycle", variable=self.compute_abs_cycle).pack(anchor="w", padx=5, pady=2)  # NEW
        ...
        # Show Data Button
        tk.Button(self.frame, text="Show Modified Data", command=self._show_modified_data).pack(pady=10)
    ...
    def _apply_modifications(self):
        ...
        if self.compute_abs_cycle.get():
            if "Cyc-Count" not in dataset.columns:
                messagebox.showerror("Error", f"Dataset is missing the 'Cyc-Count' column.")
                return

            abs_cycle = []
            current_cycle = 0

            for cycle in dataset["Cyc-Count"]:
            ...
            modified_data["abs_cycle"] = abs_cycle  # Add to dataset
