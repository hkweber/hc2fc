...
class MultiGraphPlotter:
    ...
    def plot_multi_graph(self):
        ...
        plot_type = self.app_context.multi_graph_plot_option.get()  # Use the correct variable
        ...
        try:
            if plot_type == "U-t (Voltage-Time)":
                ...
                if not charge_data.empty:
                    plt.plot(charge_data["Ah-Cyc-Charge-0"], charge_data["U[V]"], label=f"{label} (Charge)", color=color)
                
                # Plot discharge data
                if not discharge_data.empty:
                    plt.plot(discharge_data["Ah-Cyc-Discharge-0"], discharge_data["U[V]"], label=f"{label} (Discharge)", color=color, linestyle="--")
