...
class HC2FCApp:
    ...
    def _initialize_ui(self):
        ...
        # Add a button for saving the project
        tk.Button(self.main_frame, text="Save Project", command=self.save_project).pack(pady=10)
        # Add a button for loading a project
        tk.Button(self.main_frame, text="Load Project", command=self.load_project).pack(pady=10)
    ...
    def save_project(self):
        """
        Saves the current state of the application, including loaded datasets, filtered datasets, and applied filters,
        into a .hc2fcproj ZIP file.
        """
        project_data = {
            "datasets": {},
            "filtered_datasets": {},
            "applied_filters": {},  # Placeholder for storing filter configurations
            "plots": {}  # Placeholder for storing any plot-related data in the future
        }
        ...
        for dataset_type in ["anode", "cathode"]:
            ...
            if dataset is not None:
                project_data["datasets"][dataset_type] = dataset.to_csv(index=False)  # Store as CSV text

        # Save filtered datasets
        for dataset_type in ["anode", "cathode", "full_cell"]:
            project_data["filtered_datasets"][dataset_type] = []
            for filtered_set in self.filtered_datasets.get(dataset_type, []):
                project_data["filtered_datasets"][dataset_type].append({
                    "name": filtered_set["name"],
                    "data": filtered_set["data"].to_csv(index=False)
                })

        # Store applied filter settings (e.g., which checkboxes were selected)
        for dataset_type in ["anode", "cathode"]:
            filter_widget = getattr(self, f"{dataset_type}_filter_widget", None)
            if filter_widget:
                project_data["applied_filters"][dataset_type] = {
                    "remove_pause": filter_widget.remove_pause.get(),
                    "select_cycle": filter_widget.select_cycle.get(),
                    "selected_cycle": filter_widget.cycle_selection.get(),
                    "select_charge_half_cycle": filter_widget.select_charge_half_cycle.get(),
                    "select_discharge_half_cycle": filter_widget.select_discharge_half_cycle.get(),
                    "show_cycles": filter_widget.show_cycles.get(),
                    "cycle_column": filter_widget.cycle_column_option.get(),
                }
        ...
        if save_path:
            with zipfile.ZipFile(save_path, 'w', zipfile.ZIP_DEFLATED) as zf:
                # Store JSON metadata
                zf.writestr("project_metadata.json", json.dumps(project_data, indent=4))

            messagebox.showinfo("Success", f"Project saved as: {save_path}")

    def load_project(self):
        """
        Loads a saved project from a .hc2fcproj ZIP file, restoring datasets, filters, and settings.
        """
        ...
        try:
            with zipfile.ZipFile(load_path, 'r') as zf:
                if "project_metadata.json" not in zf.namelist():
                    messagebox.showerror("Error", "Invalid project file: Missing metadata.")
                    return

                # Load metadata from the project file
                project_data = json.loads(zf.read("project_metadata.json").decode("utf-8"))

            # Restore datasets
            for dataset_type in ["anode", "cathode"]:
                if dataset_type in project_data["datasets"]:
                    from io import StringIO
                    csv_data = project_data["datasets"][dataset_type]
                    self.datasets[dataset_type]["data"] = pd.read_csv(StringIO(csv_data))
                    self.datasets[dataset_type]["file_path"].set(f"Loaded from {load_path}")

            # Restore filtered datasets
            for dataset_type in ["anode", "cathode", "full_cell"]:
                self.filtered_datasets[dataset_type] = []  # Clear previous data
                for filtered_set in project_data["filtered_datasets"].get(dataset_type, []):
                    from io import StringIO
                    csv_data = filtered_set["data"]
                    restored_df = pd.read_csv(StringIO(csv_data))
                    self.filtered_datasets[dataset_type].append({
                        "name": filtered_set["name"],
                        "data": restored_df
                    })

                    # Update the respective data browser
                    if dataset_type == "anode":
                        self.anode_browser.add_dataset(filtered_set["name"])
                    elif dataset_type == "cathode":
                        self.cathode_browser.add_dataset(filtered_set["name"])
                    elif dataset_type == "full_cell":
                        self.full_cell_browser.add_dataset(filtered_set["name"])

            # Restore filter settings
            for dataset_type in ["anode", "cathode"]:
                if dataset_type in project_data["applied_filters"]:
                    widget = getattr(self, f"{dataset_type}_filter_widget", None)
                    if widget:
                        settings = project_data["applied_filters"][dataset_type]
                        widget.remove_pause.set(settings["remove_pause"])
                        widget.select_cycle.set(settings["select_cycle"])
                        widget.cycle_selection.set(settings["selected_cycle"])
                        widget.select_charge_half_cycle.set(settings["select_charge_half_cycle"])
                        widget.select_discharge_half_cycle.set(settings["select_discharge_half_cycle"])
                        widget.show_cycles.set(settings["show_cycles"])
                        widget.cycle_column_option.set(settings["cycle_column"])

            messagebox.showinfo("Success", f"Project loaded from: {load_path}")
