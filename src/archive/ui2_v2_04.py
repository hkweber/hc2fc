...
class HC2FCApp:
    ...
    def plot_filtered_data(self, dataset_type):
        """
        Plots the filtered data for the selected dataset type.
        """
        ...
        try:
            ...
            if plot_type == "U-t":
                plt.plot(filtered_data["Time[h]"], filtered_data["U[V]"], label="Voltage-Time")
                plt.xlabel("Time (h)")
                plt.ylabel("Voltage (V)")
                plt.title(f"{dataset_type.capitalize()} Data: U vs t")

            elif plot_type == "I-t":
                plt.plot(filtered_data["Time[h]"], filtered_data["I[A]"], label="Current-Time", color="orange")
                plt.xlabel("Time (h)")
                plt.ylabel("Current (A)")
                plt.title(f"{dataset_type.capitalize()} Data: I vs t")

            elif plot_type == "Q-U":
                charge_column = "Ah-Cyc-Charge-0" if widget.select_charge_half_cycle.get() else "Ah-Cyc-Discharge-0"
                plt.plot(filtered_data[charge_column], filtered_data["U[V]"], label="Charge-Voltage", color="green")
                plt.xlabel("Charge (Ah)")
                plt.ylabel("Voltage (V)")
                plt.title(f"{dataset_type.capitalize()} Data: Q vs U")
