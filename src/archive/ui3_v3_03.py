...
class MultiGraphPlotter:
    ...
    def plot_multi_graph(self):
        ...
        and plot them in a single figure based on the selected plot type.
        """
        ...
        # Get the selected plot type from the new dropdown
        ...
        for dataset_name in anode_datasets:
            dataset = self._get_dataset_by_name(dataset_name, "anode")
            if dataset is not None:
                self._plot_dataset(dataset, dataset_name, plot_type, "blue")

        # Plot selected datasets from the cathode browser
        for dataset_name in cathode_datasets:
            dataset = self._get_dataset_by_name(dataset_name, "cathode")
            if dataset is not None:
                self._plot_dataset(dataset, dataset_name, plot_type, "red")

        plt.xlabel(self._get_xlabel(plot_type))
        plt.ylabel(self._get_ylabel(plot_type))
        plt.title("Multi-Graph Plot")
        plt.legend()
        plt.grid(True)
        plt.show()
    ...
    def _get_dataset_by_name(self, dataset_name, dataset_type):
        ...
        """
        datasets = self.app_context.filtered_datasets[dataset_type]
        for dataset in datasets:
            if dataset["name"] == dataset_name:
                return dataset["data"]
        return None

    def _plot_dataset(self, dataset, label, plot_type, color):
        """
        Plot a single dataset on the current matplotlib figure based on the selected plot type.
        """
        try:
            if plot_type == "U-t (Voltage-Time)":
                plt.plot(dataset["Time[h]"], dataset["U[V]"], label=label, color=color)
            elif plot_type == "I-t (Current-Time)":
                plt.plot(dataset["Time[h]"], dataset["I[A]"], label=label, color=color)
            elif plot_type == "Q-U (Charge-Voltage)":
                charge_column = (
                    "Ah-Cyc-Charge-0" if "Ah-Cyc-Charge-0" in dataset.columns else "Ah-Cyc-Discharge-0"
                )
                plt.plot(dataset[charge_column], dataset["U[V]"], label=label, color=color)
    ...
    def _get_xlabel(self, plot_type):
        """
        Returns the appropriate X-axis label based on the plot type.
        """
        if plot_type == "U-t (Voltage-Time)" or plot_type == "I-t (Current-Time)":
            return "Time (h)"
        elif plot_type == "Q-U (Charge-Voltage)":
            return "Charge (Ah)"
        return "X-axis"

    def _get_ylabel(self, plot_type):
        """
        Returns the appropriate Y-axis label based on the plot type.
        """
        if plot_type == "U-t (Voltage-Time)" or plot_type == "Q-U (Charge-Voltage)":
            return "Voltage (V)"
        elif plot_type == "I-t (Current-Time)":
            return "Current (A)"
        return "Y-axis"
...
class HC2FCApp:
    ...
    def _initialize_ui(self):
        ...
        self.multi_graph_plot_option = tk.StringVar(value="U-t")  # Default to U-t
    ...
    def apply_filters(self, dataset_type):
        ...
        if widget.select_cycle.get():
            ...
            if selected_cycle and selected_cycle != "All":
                ...
                messagebox.showinfo("Info", "Showing all cycles (no specific cycle selected).")
