...
class ModifyDataWidget:
    ...
    def _apply_modifications(self):
        ...
        if self.compute_abs_cycle.get():
            modified_data = self.app_context.compute_absolute_cycle(modified_data)  # Use centralized method
...
class MultiGraphPlotter:
    ...
    def plot_multi_graph(self):
        """
        Fetch selected datasets from both anode and cathode browsers
        ...
        if not anode_datasets and not cathode_datasets:
            messagebox.showerror("Error", "No datasets selected from either browser.")
            return
        ...
        # Plot selected datasets from the anode browser
        for dataset_name in anode_datasets:
            dataset = self.app_context._get_dataset_by_name(dataset_name, "anode")  # Updated to use HC2FCApp
            if dataset is not None:
                self._plot_dataset(ax1, ax2, dataset, dataset_name, plot_type, "blue")

        # Plot selected datasets from the cathode browser
        for dataset_name in cathode_datasets:
            dataset = self.app_context._get_dataset_by_name(dataset_name, "cathode")  # Updated to use HC2FCApp
            if dataset is not None:
                self._plot_dataset(ax1, ax2, dataset, dataset_name, plot_type, "red")

        # Plot selected datasets from the full cell browser
...
class HC2FCApp:
    ...
    def compute_absolute_cycle(dataset):
        """
        Compute the absolute cycle count from 'Cyc-Count'.
        - Cycles 1-N remain unchanged if no reset occurs.
        - If 'Cyc-Count' resets to a lower value, increment abs_cycle **only once**.
        - Ensure continuous cycle counting.
        """
        if "Cyc-Count" not in dataset.columns:
            messagebox.showerror("Error", "Dataset is missing the 'Cyc-Count' column.")
            return dataset  # Return original dataset to avoid breaking flow

        abs_cycle = []
        current_abs_cycle = 1  # Start absolute cycle numbering at 1
        previous_cyc_count = dataset["Cyc-Count"].iloc[0]

        for cyc_count in dataset["Cyc-Count"]:
            if cyc_count < previous_cyc_count:  # Reset detected (drop in cycle count)
                current_abs_cycle += 1  # Increment only once per reset

            abs_cycle.append(current_abs_cycle)
            previous_cyc_count = cyc_count  # Update previous cycle count

        dataset["abs_cycle"] = abs_cycle  # Store computed column
        return dataset
    ...
    def apply_filters(self, dataset_type):
        ...
        if modify_widget.compute_abs_cycle.get():
            if "Cyc-Count" in filtered_data.columns:
                abs_cycle = []
                current_cycle = None  # Track the last seen cycle number
                abs_cycle_number = 1   # Start absolute cycle numbering at 1

                for i, cycle in enumerate(filtered_data["Cyc-Count"]):
                    if current_cycle is None:
                        # Initialize first cycle
                        current_cycle = cycle
                    elif cycle < current_cycle:
                        # If cycle count resets (drops), increment absolute cycle number
                        abs_cycle_number += 1
                    # Else: Keep the same cycle number
                    
                    abs_cycle.append(abs_cycle_number)
                    current_cycle = cycle  # Update the current cycle tracker

                filtered_data["abs_cycle"] = abs_cycle  # Add corrected column
        ...
        return filtered_data  # Return modified dataset
