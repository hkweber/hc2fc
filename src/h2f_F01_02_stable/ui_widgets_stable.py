...
class FilterWidget:
    ...
    def _create_filter_options(self):
        ...
        # Select Cycle Frame, Checkbutton and Dropdown
        ...
        self.cycle_column_dropdown = tk.OptionMenu(
            cycle_frame, self.cycle_column, "Cyc-Count", "abs_cycle", command=self._update_selected_cycles
        )
...
#        cycles_frame.pack(anchor="w", padx=UIStyling.LISTBOX_PADX, pady=2, fill="x")
        ...
        # Cycle Column Dropdown (used in filter and in visualization area) 
...
#        self.cycle_column_dropdown = tk.OptionMenu(cycles_frame, self.cycle_column, "Cyc-Count", "abs_cycle")
#        self.cycle_column_dropdown.config(font=UIStyling.DROPDOWN_FONT)
#        self.cycle_column_dropdown.pack(side="left", padx=UIStyling.DROPDOWN_PADX)
...
### Updates for dropdown menus
    ...
    def _update_selected_cycles(self, selected_column):
        """
        Updates the cycle selection dropdown based on the selected cycle column.
        """
        dataset = self.app_context.data_manager.get_dataset(self.dataset_type)
    ...
    def _toggle_cycle_column_dropdown(self):   #hkw
        """
        Enable or disable the cycle column dropdown based on the Select Cycle checkbox.
        """
        if self.select_cycle.get():
            self.cycle_dropdown.config(state="normal")
        else:
