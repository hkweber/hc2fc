...
#        self.cycle_column_dropdown.config(font=UIStyling.DROPDOWN_FONT)
    ...
    '''
        def _update_cycle_column_dropdown(self, selected_column):
            ...
            if selected_column not in dataset.columns:
    ...
    def _update_cycle_column_dropdown(self, selected_column):
        """
        Updates the cycle_column selection dropdown based on the selected cycle column.
        Ensures that the selected column exists before attempting to update.
        """
        dataset = self.app_context.data_manager.get_dataset(self.dataset_type)
        ...
        # ✅ Check if the selected column exists before accessing it
        if selected_column not in dataset.columns:
            messagebox.showwarning("Warning", f"Column '{selected_column}' not found. Creating it now.")
            
            # ✅ Try computing `abs_cycle` before proceeding
            if selected_column == "abs_cycle":
                dataset = self.app_context.data_manager.compute_absolute_cycle(dataset)
                self.app_context.data_manager.datasets[self.dataset_type]["data"] = dataset  # ✅ Update dataset storage

            # ✅ Automatically enable the Compute Absolute Cycle checkbox
            if self.dataset_type in self.app_context.modify_widgets:
                modify_widget = self.app_context.modify_widgets[self.dataset_type]
                modify_widget.compute_abs_cycle.set(True)  # ✅ Toggle the checkbox

            # ✅ Recheck if column exists
            if selected_column not in dataset.columns:
                messagebox.showerror("Error", f"Column '{selected_column}' could not be created.")
                return

        # ✅ Now it's safe to access the column
        unique_cycles = sorted(dataset[selected_column].dropna().unique())
    ...
    def _update_cycle_column_dropdown(self, selected_column):
        """
        Updates the cycle_column selection dropdown based on the selected cycle column.
        If 'abs_cycle' is selected, it automatically toggles 'Compute Absolute Cycle' in ModifyDataWidget.
        """
        dataset = self.app_context.data_manager.get_dataset(self.dataset_type)
        ...
        # ✅ Check if the selected column exists before accessing it
        if selected_column not in dataset.columns:
            messagebox.showwarning("Warning", f"Column '{selected_column}' not found.")

        # ✅ If the user selects 'abs_cycle', auto-toggle the 'Compute Absolute Cycle' checkbox
        if selected_column == "abs_cycle":
            if self.dataset_type in self.app_context.modify_widgets:
                modify_widget = self.app_context.modify_widgets[self.dataset_type]
                modify_widget.compute_abs_cycle.set(True)  # ✅ Toggle the checkbox
...
#            self.cycle_column_dropdown.config(state="normal")
    ...
    def toggle_cycle_column_dropdown(self):
        ...
        state = "normal" if modify_widget.compute_abs_cycle.get() else "disabled"
        self.cycle_column_dropdown.config(state=state)    
