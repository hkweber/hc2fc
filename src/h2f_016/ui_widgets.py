...
class DataWidget:
    ...
    def __init__(self, parent, label_text, dataset_key, app_context):
        ...
        self.file_label = tk.Label(label_frame, textvariable=self.app_context.datasets[dataset_key]["file_path"], width=40, anchor="w")
        ...
        tk.Button(button_frame, text="Load", command=lambda: self.data_manager.select_file(self.dataset_key), font=UIStyling.BUTTON_FONT).pack(side="left", padx=5, pady=2)
        tk.Button(button_frame, text="Show Data Table", command=lambda: self.data_manager.show_data_table(self.dataset_key), font=UIStyling.BUTTON_FONT).pack(side="left", padx=5, pady=2)
        tk.Button(button_frame, text="Plot Data", command=lambda: self.data_manager.plot_data(self.dataset_key), font=UIStyling.BUTTON_FONT).pack(side="left", padx=5, pady=2)
