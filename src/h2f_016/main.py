...
class HC2FCApp:
    ...
    def _create_data_processing_section(self):
        ...
        '''
                # ✅ **Restore Buttons for Plot, Store, and Save**
        button_frame = tk.Frame(data_processing_frame)
        button_frame.pack(fill="x", padx=UIStyling.FRAME_PADX, pady=UIStyling.FRAME_PADY)

        # ✅ Plot Filtered Data
        tk.Button(
            button_frame, text="Plot Filtered Data",
            command=lambda: self.data_manager.plot_filtered_data("anode"),
            font=UIStyling.BUTTON_FONT
        ).pack(side="left", padx=UIStyling.PAD_X)

        tk.Button(
            button_frame, text="Plot Filtered Data",
            command=lambda: self.data_manager.plot_filtered_data("cathode"),
            font=UIStyling.BUTTON_FONT
        ).pack(side="left", padx=UIStyling.PAD_X)

        # ✅ Store Filtered Data
        tk.Button(
            button_frame, text="Store Filtered Data",
            command=lambda: self.data_manager.store_filtered_data("anode"),
            font=UIStyling.BUTTON_FONT
        ).pack(side="left", padx=UIStyling.PAD_X)

        tk.Button(
            button_frame, text="Store Filtered Data",
            command=lambda: self.data_manager.store_filtered_data("cathode"),
            font=UIStyling.BUTTON_FONT
        ).pack(side="left", padx=UIStyling.PAD_X)

        # ✅ Save Filtered Data
        tk.Button(
            button_frame, text="Save Filtered Data",
            command=lambda: self.data_manager.save_filtered_data("anode"),
            font=UIStyling.BUTTON_FONT
        ).pack(side="left", padx=UIStyling.PAD_X)

        tk.Button(
            button_frame, text="Save Filtered Data",
            command=lambda: self.data_manager.save_filtered_data("cathode"),
            font=UIStyling.BUTTON_FONT
        ).pack(side="left", padx=UIStyling.PAD_X)
    ...
    def plot_filtered_data(self, dataset_type):
        ...
        - 'Show Key Values'
        """
        filtered_data = self.data_manager.apply_filters(dataset_type)  # ✅ Use DataManager
