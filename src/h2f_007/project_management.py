...
class ProjectManager:
    ...
    def save_project(self, project_path):
        ...
        try:
            project_data = {
                "original_files": {
                    "anode": self.app.datasets["anode"]["file_path"].get(),
                    "cathode": self.app.datasets["cathode"]["file_path"].get()
                },
                "datasets": {},
                "filters": {},
                "plots": []
            ...
            for dataset_type, datasets in self.app.filtered_datasets.items():
            ...
            with zipfile.ZipFile(project_path, "w", zipfile.ZIP_DEFLATED) as project_zip:
                ...
                for dataset_type in ["anode", "cathode"]:
                    dataset_df = self.app.datasets[dataset_type]["data"]
                    if dataset_df is not None:
                        dataset_csv = dataset_df.to_csv(index=False)
                        project_zip.writestr(f"datasets/{dataset_type}.csv", dataset_csv)
                
                '''
                # ✅ Step 3: Update UI file paths for anode/cathode main datasets
                if self.app.datasets["anode"]["data"] is not None:
                    filename = self.app.datasets["anode"]["file_path"].get()
                    self.app.anode_widget.file_label.config(text=f"Anode file: {filename}")

                if self.app.datasets["cathode"]["data"] is not None:
                    filename = self.app.datasets["cathode"]["file_path"].get()
    ...
    def load_project(self, project_path):
        ...
        try:
            with zipfile.ZipFile(project_path, "r") as project_zip:
                ...
                self.app.filtered_datasets = {"anode": [], "cathode": [], "full_cell": []}
                self.app.modified_datasets = {"anode": None, "cathode": None}

                # ✅ Load datasets
                for dataset_type in ["anode", "cathode"]:
                    if f"datasets/{dataset_type}.csv" in project_zip.namelist():
                        with project_zip.open(f"datasets/{dataset_type}.csv") as dataset_file:
                            dataset_df = pd.read_csv(dataset_file)
                            self.app.datasets[dataset_type]["data"] = dataset_df
                            self.app.datasets[dataset_type]["file_path"].set(dataset_type)

                # ✅ Restore filtered datasets
                for dataset_type, dataset_list in project_data["datasets"].items():
                    for dataset_name in dataset_list:
                        if f"datasets/{dataset_name}.csv" in project_zip.namelist():
                            with project_zip.open(f"datasets/{dataset_name}.csv") as dataset_file:
                                dataset_df = pd.read_csv(dataset_file)
                                self.app.filtered_datasets[dataset_type].append(
                                    {"name": dataset_name, "data": dataset_df}
                                )

                # ✅ Restore UI
                for dataset in self.app.filtered_datasets["anode"]:
                    self.app.anode_browser.add_dataset(dataset["name"])
                for dataset in self.app.filtered_datasets["cathode"]:
    ...
    def load_project(self, project_path):
        ...
        try:
            with zipfile.ZipFile(project_path, "r") as project_zip:
                ...
                for dataset_type in ["anode", "cathode"]:
                    ...
                    if original_filename and f"datasets/{original_filename}" in project_zip.namelist():
                        with project_zip.open(f"datasets/{original_filename}") as dataset_file:
                            dataset_df = pd.read_csv(dataset_file)
                            self.app.datasets[dataset_type]["data"] = dataset_df
                            self.app.datasets[dataset_type]["file_path"].set(original_filename)  # ✅ Update file path
                ...
                for dataset in self.app.filtered_datasets["cathode"]:
                    self.app.cathode_browser.add_dataset(dataset["name"])
                for dataset in self.app.filtered_datasets["full_cell"]:
                ...
                # ✅ Step 3: Update UI file paths for anode/cathode main datasets
                if self.app.datasets["anode"]["data"] is not None:
                    self.app.anode_widget.file_label.config(text=self.app.datasets["anode"]["file_path"].get())

                if self.app.datasets["cathode"]["data"] is not None:
                    self.app.cathode_widget.file_label.config(text=self.app.datasets["cathode"]["file_path"].get())
                ...
                if self.app.datasets["anode"]["data"] is not None:
                    self.app.anode_widget.file_label.config(text=self.app.datasets["anode"]["file_path"].get())
                    self.app.anode_widget.file_path.set(self.app.datasets["anode"]["file_path"].get())  # ✅ Update UI variable

                if self.app.datasets["cathode"]["data"] is not None:
                    self.app.cathode_widget.file_label.config(text=self.app.datasets["cathode"]["file_path"].get())
                    self.app.cathode_widget.file_path.set(self.app.datasets["cathode"]["file_path"].get())  # ✅ Update UI variable
