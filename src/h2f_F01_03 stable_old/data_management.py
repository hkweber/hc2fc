...
### filter methods
    ...
    def apply_filters(self, dataset_type):
        ...
        # generate fit data if "Fit data" is selected in data_type_dropdown
        if widget.data_type_selection.get() == "Fit Data":
            # Handle Fit Data generation
            if widget.fit_option.get() != "linear spline":
                messagebox.showerror("Error", "Linear spline fit must be selected to generate Fit Data.")
                return None
            
            if widget.plot_option.get() != "Q-U":
                messagebox.showerror("Error", "Can't generate 'U vs t' or 'I vs t' fit data. Please select 'Q vs U'.")
                return None
            
            # Generate Fit Data for Q vs U
            try:
                fit_data = self._generate_linear_spline_fit_data(data) #hkw wrong?
                return fit_data
            except Exception as e:
                messagebox.showerror("Error", f"Failed to generate fit data: {e}")
                return None
        else:
            # Modify Data if "modified Data" is selected in data_type_dropdown 
...
### fit methods
    ...
    def _generate_linear_spline_fit_data(self, dataset):
        """
        Generate a linear spline fit dataset for Q vs U.
        """
        # Ensure necessary columns are present
        required_columns = ["Q(Ah)", "U(V)"]
        if not all(col in dataset.columns for col in required_columns):
            raise ValueError(f"Dataset must contain columns: {', '.join(required_columns)}")

        # Sort and interpolate
        sorted_data = dataset.sort_values(by="Q(Ah)").reset_index(drop=True)
        interpolated_data = sorted_data[["Q(Ah)", "U(V)"]].copy()

        # Add a row counter
        interpolated_data["row"] = range(len(interpolated_data))
...
### suffix creation for data selection filtered/fit data
    ...
    def _generate_datatype_suffix(self, dataset_type):
        ...
        if filter_widget.data_type_selection.get() == "Fit Data":
            return "_FitData_linSpline"

        return ""  # Default: No suffix for modified data
